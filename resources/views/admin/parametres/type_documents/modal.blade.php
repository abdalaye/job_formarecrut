...
<div class="modal fade" id="creationModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.typedocuments.store') }}" enctype="multipart/form-data">
                <div class="modal-body">
                    @csrf
                    <div class="row">
                        <div class="form-group col-12">
                            <label for="name">Libellé du type de document</label>
                                ...
                                <div class="d-block">
                                    ...
                                    @foreach ($extensions as $name => $values)
                                        <label for="{{ $name }}" class="p-2 mr-2 border rounded font-weight-normal">
                                            <input {{ $i < 3 ? "checked" : "" }} type="checkbox" name="authorised_extensions[]" value="{{ $name }}" id="{{ $name }}">
                                            {{ strtoupper($name) }}
                                        </label>
                                        @php
                                         $i++;
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="" id="editForm" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="form-group col-12">
                            <label for="edit_name">Nature du document</label>
                        ...
                        <div class="form-group col-12">
                            ...
                            @if (isset($extensions) && count($extensions))
                                <div class="d-block">
                                    @foreach ($extensions as $name => $values)
                                        <label for="{{ $name }}" class="p-2 mr-2 border rounded font-weight-normal">
                                            <input type="checkbox" name="authorised_extensions[]" value="{{ $name }}" id="{{ $name }}">
                                            {{ strtoupper($name) }}
                                        </label>
                                    @endforeach
                                </div>
