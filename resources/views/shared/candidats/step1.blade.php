<form action="{{ route('admin.candidats.step1', $candidat) }}" enctype="multipart/form-data" method="post" class="row">
    @csrf
    @method("PATCH")
    <div class="col-md-12">
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="photo_upload" class="d-block {{ $candidat->photo ? 'border p-3' : 'jumbotron' }} text-center">
                        @if ($candidat->photo)
                            <a href="{{ asset($candidat->photo) }}" data-fancybox="Profil du candidat">
                                <img src="{{ asset($candidat->photo) }}" class="img-thumbnail img-rounded shadow img-size-64 d-block mx-auto" alt="Profil candidat">
                            </a>
                        @endif
                        <span class="text-muted choosePhoto">Cliquez pour choisir un photo</span>
                        <span class="text-success photoChoosed d-none"> <i class="fas fa-check mr-2"></i> Une image a été bien choisie</span>
                        <p>
                            <small class="small text-center text-muted">
                                Pour un meilleur rendu, veuillez choisir une photo au format carré.
                            </small>
                        </p>
                        <input accept=".png,.jpeg,.jpg" type="file" name="photo_upload" class="form-control form-control-sm form-control-file d-none" id="photo_upload">
                    </label>
                </div>
            </div>
            <div class="col-md-6">
