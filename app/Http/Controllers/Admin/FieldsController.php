    ...
    public function store(FieldRequest $request)
    {
        ...
        if($field->save()) {
            ...
            Log::ACTION_GENEGAL("Création nouveau champ dynamique",
                    "L'admnistrateur " . Auth::user()->nom_complet . " a créé un nouveau champ : $field->name pour le type de document ".$field->type_document->name.".");
    ...
    public function ranger(Request $request, $type_document_id) {
        ...
        if(count($fields) > 0) {
            ...
            Log::ACTION_GENEGAL("Rangement champs dynamiques",
                    "L'admnistrateur " . Auth::user()->nom_complet . " a rangé les champs pour le type de document ".$type_document->name.".");
    ...
    public function update(Request $request, Field $field)
    {
        if($field->update(request()->all())) {
            ...
            Log::ACTION_GENEGAL("Modification champ dynamique",
                "L'admnistrateur " . Auth::user()->nom_complet . " a modifé le champ : $field->name pour le type de document ".$field->type_document->name.".");
        } else {
    ...
    public function destroy(Field $field)
    {
        if($field->delete()) {
            ...
            Log::ACTION_GENEGAL("Suppression champ dynamique",
                "L'admnistrateur " . Auth::user()->nom_complet . " a supprimé le champ : $field->name pour le type de document ".$field->type_document->name.".");
